...
import com.lcsd.luxi.ui.adapter.ZTZL_Head_adapter;
...
public class ZTZLActivity extends BaseActivity {
    private Context mContext;
    private String url, title, mark;
    private List<ZTZL_Head_Info.TContent> list;
    private ZTZL_Head_adapter adapter;
    @BindView(R.id.psts_tab_ztzl)
    PagerSlidingTabStrip tabs;
    @BindView(R.id.ztzl_view_pager)
    ViewPager viewPager;
    @BindView(R.id.ztzl_top_view)
    View mView;
    @BindView(R.id.tv_ztzl_title)
    TextView tv_title;
    @BindView(R.id.ll_ztzl_back)
    LinearLayout ll_back;
    @BindView(R.id.statu_ztzl)
    MultipleStatusView statusView;

    @Override
    protected int setLayoutId() {
        return R.layout.activity_ztzl;
    }
        ...
        MyApplication.getInstance().getmMyOkHttp().post(mContext, url, null, new RawResponseHandler() {
            ...
            public void onSuccess(int statusCode, String response) {
                if (response != null) {
                    ...
                    LogUtil.d(response);
                    ZTZL_Head_Info info = JSON.parseObject(response, ZTZL_Head_Info.class);
                    if (info.getContent() != null && info.getContent().size() > 0) {
                        list.addAll(info.getContent());
                    }
                    if (list.size() > 0) {
                        adapter = new ZTZL_Head_adapter(getSupportFragmentManager(), list, mark);
