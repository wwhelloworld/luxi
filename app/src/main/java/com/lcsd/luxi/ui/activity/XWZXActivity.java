...
public class XWZXActivity extends BaseActivity {
    ...
    private XWZX_Head_adapter adapter;
    @BindView(R.id.psts_tab_xwzx)
    PagerSlidingTabStrip tabs;
    @BindView(R.id.xwzx_view_pager)
    ViewPager viewPager;
    @BindView(R.id.xwzx_top_view)
    View mView;
    @BindView(R.id.tv_xwzx_title)
    TextView tv_title;
    @BindView(R.id.ll_xwzx_back)
    LinearLayout ll_back;
    @BindView(R.id.statu_xwzx)
        ...
        MyApplication.getInstance().getmMyOkHttp().post(mContext, url, null, new RawResponseHandler() {
            ...
            public void onSuccess(int statusCode, String response) {
                if (response != null) {
                    try {
                    LogUtil.d(response);
                    XWZX_Head_Info info = JSON.parseObject(response, XWZX_Head_Info.class);
                    if (info.getContent() != null && info.getContent().size() > 0) {
                        list.addAll(info.getContent());
                    }
                    if (list.size() > 0) {
                        adapter = new XWZX_Head_adapter(getSupportFragmentManager(), list, mark);
